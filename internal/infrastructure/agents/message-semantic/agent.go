...
func New() *Agent {
	...
	agent, err := llmagent.New(llmagent.Config{
		...
		Запрещается выдумывать любую информацию. Если не знаешь — лучше пропустить. По возможности нужно указать для всех сообщений.`,

		// 		Instruction: `Ты — агент, который анализирует сообщения из мессенджера.

		// Тебе будут даны сообщения из одного чата, название чата и его описание.
		...
		//   "messages": [
		//     {"message_id": "id1", "sender_id": "...", "content": "текст1", ...},
		//     {"message_id": "id2", "sender_id": "...", "content": "текст2", ...}
		//   ]
		// }

		// Твоя задача — для КАЖДОГО сообщения определить:
		// 1. domain (предметная область): массив строк
		// 2. entities (конкретика): массив строк
		// 3. intent (намерение): массив строк

		// ФОРМАТ ОТВЕТА (строго JSON массив, без пояснений):
		// [
		//   {
		//     "message_id": "id1",
		//     "domain": ["IT", "технологии"],
		//     "entities": ["Claude", "AI"],
		//     "intent": ["вопрос", "помощь"]
		//   },
		//   {
		//     "message_id": "id2",
		//     "domain": ["спорт"],
		//     "entities": ["WNBA", "Cleveland"],
		//     "intent": ["обсуждение", "информация"]
		//   }
		// ]

		// ВАЖНЫЕ ПРАВИЛА:
		// 1. Для КАЖДОГО message_id из входных данных должен быть элемент в массиве
		// 2. Используй ТОЧНЫЕ message_id, которые даны во входных данных
		// 3. Если тегов несколько - массив, если один - массив с одним элементом
		// 4. ОТВЕТЬ ТОЛЬКО JSON массивом, БЕЗ пояснений и маркдауна

		// Возможные значения для domain: развлечения, автомобили, IT, медицина, новости, техника, путешествия, спорт, образование, финансы, еда, здоровье, политика, наука, искусство, бизнес, работа, дом, семья, хобби, игры, культура
		// Возможные значения для intent: вопрос, ответ, покупка, продажа, обсуждение, шутка, согласие, несогласие, просьба, предложение, жалоба, похвала, критика, совет, инструкция, приветствие, прощание, реклама, спам
